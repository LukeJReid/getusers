...
def options():
    ...
    parser.add_argument('-v', '--version', dest='show_version', action='store_true', help='Shows script version')
    
    parser.add_argument('-s', '--system-users', dest='show_system_users', action='store_true', help='Show system users on the device')

    parser.add_argument('-u', '--users', dest='show_users', action='store_true', help='Show users on this device')
    
    parser.add_argument('-a', '--all-users', dest='show_all_users', action='store_true', help='Show all users on this device. This is the default option')

    parser.add_argument('-F', '--show-full', dest='show_full_output', action='store_true', help='When displaying, show the full user information including GECOS and Group ID')
    return parser
...
class Config(object):
    ...
    HEADER_FULL = ['ID', 'User', 'Group ID', 'GECOS', 'Home', 'Shell', 'Sudo', 'Last Login']
...
def init_variables():
    ...
    p = subprocess.Popen(Config.LAST_CMD + Config.WTMP_FILE, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def get_system_full():
    ...
    for x in Users.USERS:
        if(x[2] <= Users.SYS_UID_MAX):
            ...
            if(gecos == ""):
                gecos = "None"            
            users_table.append([x[2], x[0], x[3], gecos, x[5], x[6], sudo, last_login])
    return users_table
...
def print_table(headers, table):
    ...
    print(Color.GREEN + "".join(str(word).ljust(column_width) for word in headers), Color.RESET)
    for x in table:
        print(Color.CYAN + "".join(str(word).ljust(column_width) for word in x) , Color.RESET)
